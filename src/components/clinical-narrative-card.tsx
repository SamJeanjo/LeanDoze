...
import type { NarrativeSection, PatientNarrative } from "@/lib/report-narrative";

export function ClinicalNarrativeCard({
  narrative,
  sections,
  days,
  hasData,
  ...
  const sectionMap = new Map(sections.map((section) => [section.title, section.items]));
  const overall = sectionMap.get("Overall") ?? [];
  const keyPatterns = sectionMap.get("Key patterns") ?? [];
  const discussionPoints = narrative.discussionPoints.length ? narrative.discussionPoints : sectionMap.get("Discussion points") ?? [];
  const disclaimer = sectionMap.get("Disclaimer")?.[0] ?? "This summary is for clinical review. LeanDoze does not provide medical advice.";
      ...
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-start">
        <div className="max-w-3xl">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-[#64748B]">
            A concise interpretation of recent adherence, nutrition, hydration, symptoms, energy, and flags for clinical review.
          </p>
        </div>

        <div className="flex flex-wrap gap-2 lg:justify-end">
          <CopySummaryButton text={narrative.summary} />
          {addToReportAction}
          {generateThirtyDayAction}
        </div>
      </div>
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-[1fr_1.25fr]">
          <div className="rounded-[22px] border border-[#E2E8F0]/80 bg-[#F8FAFC]/75 p-5">
            ...
            <div className="mt-4 space-y-2">
              {overall.map((item) => (
                <p key={item} className="text-sm leading-6 text-[#334155]">{item}</p>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-[22px] border border-[#E2E8F0]/80 bg-white p-5">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {keyPatterns.slice(0, 8).map((item) => (
