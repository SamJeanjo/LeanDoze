...
export function QuickLogSheet({ type, onClose }: QuickLogSheetProps) {
  ...
  return (
    <div className="fixed inset-0 z-[90]">
      <button className="absolute inset-0 bg-[#07111F]/35 backdrop-blur-sm" aria-label="Close quick log" onClick={onClose} />
      <section className="absolute inset-x-0 bottom-0 rounded-t-[32px] bg-white p-5 shadow-[0_-28px_80px_rgba(7,17,31,0.25)] sm:left-1/2 sm:max-w-lg sm:-translate-x-1/2 sm:rounded-[32px]">
        <div className="mx-auto h-1.5 w-12 rounded-full bg-[#CBD5E1]" />
        <div className="mt-5 flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#0F766E]">Quick log</p>
            <h2 className="mt-2 text-2xl font-semibold tracking-[-0.035em] text-[#07111F]">{type}</h2>
          </div>
